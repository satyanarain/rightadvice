...
add_action( 'admin_menu', 'lawyer_menu' );
function lawyer_menu(){
    add_menu_page( 'All Lawyers', 'All Lawyers', 'manage_options', 'all_lawyer', 'all_lawyer_page', '' ,6 );
}
...
function user_menu(){
    add_menu_page( 'All Users', 'All Users', 'manage_options', 'all_user', 'all_user_page', '' ,7);
}
...
function question_answer(){
    add_menu_page( 'Questions & Answers', 'Questions & Answers', 'manage_options', 'question_answer', 'all_question_answer_page', '' ,8);
}
...
function rating_comments(){
    add_menu_page( 'Rating & Comments', 'Rating & Comments', 'manage_options', 'rating_comment', 'all_rating_comment_page', '' ,9);
}
...
function report(){
    add_menu_page( 'Average Response Time Report', 'Average Response Time Report', 'manage_options', 'report', 'all_report', '' ,9);
}
...
function get_breadcrumb() {
    echo '<i class="fa fa-home pr-10"></i><a href="'.home_url().'" rel="nofollow">Home</a>';
...
function rightadvice_custom_excerpt_length( $length ) {
   return 20;
}
add_filter( 'excerpt_length', 'rightadvice_custom_excerpt_length', 999 );

//add more link to excerpt
function rightadvice_custom_excerpt_more($more) {
   global $post;
   return '<a class="more-link" href="'. get_permalink($post->ID) . '">'. __('...', 'rightadvice') .'</a>';
}
add_filter('excerpt_more', 'rightadvice_custom_excerpt_more');


//function my_cronjob_action () {
   // global $wpdb;
	/* $result = $wpdb->get_results("select ra_question.*, ra_lawyers.id, ra_lawyers.email
								  from ra_question 
								  join ra_lawyers on ra_lawyers.id = ra_question.lawyer_id
								  where"); */
								  
	/* $to = "sandytechnology2020@gmail.com";
	$subject = "Cron job Mail From RightAdvice";
	$message = '<h3>Dear Sandeep,</h3>
				<p>You received this mail sending in every minuts</p>';
	...
	if(mail($to,$subject,$message,$headers)){
		echo "Reminder mail is send successfully.";
	}else{
