...
<h3>All Users <br><br><!--a href="add-lawyer.php" class="btn btn-info">Add New Lawyer</a--></h3>
...
<table id="example" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
	...
	<?php 
		$sno = 0;
		$query = mysql_query("select * from ra_front_users order by added_date desc") or die(mysql_error());
	...
	?>
		<tr id="item_<?=$rows['id']?>">
			...
			<td><?=$rows['email']?></td>
			<td>
				<?php if($rows['email_confirm'] == 0){ ?>
					<span class="label label-danger">Email Not Confirm</span>
				<?php }else if($rows['email_confirm'] == 1){?>
					<span class="label label-success">Email Confirm</span>
				<?php }?>
			</td>
			<td><?=date('d F Y', strtotime($rows['added_date']))?></td>
			<td>
				<!--a href="add-lawyer.php?lid=<?=$rows['id']?>" class="btn btn-info"> <i class="fa fa-pencil"></i> </a-->
				<a href="javascript:void(0)" onclick="deleteThis('<?=$rows['id']?>','user')" class="btn btn-danger"> <i class="fa fa-trash"></i> </a>
			</td>
		</tr>
	<?php }?>
	</tbody>
